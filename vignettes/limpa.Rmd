...
date: "First version 30 December 2024, Last revised 31 January 2026"
abstract: >
  ...
  The limpa package implements a pipeline for quantification and differential expression analysis of mass-spectrometry (MS) proteomics data, with probabilistic recovery of information from missing values.
...
limpa uses the DPC, together with a Bayesian model, to summarize the precursor quantifications into protein-level quantifications and to undertake limma-based differential expression analyses (Li 2024, Ritchie et al 2015).
...
(Rows can alternatively correspond to peptides, proteins or PTMs, see below).
...
In each case, limpa directly reads files output by those tools without any need for prior processing by the analyst.
...
# Differential precursor or peptide usage

A third application for which `dpcQuantByRow` is used instead of `dpcQuant` is for detecting differential isoform usage.
To detect differential precursor usage for each protein between experimental conditions, one fits precursor-level linear models using `dpcDE`, then `limma::diffSplice` tests for differential usage.
