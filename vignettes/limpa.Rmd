...
date: "First version 30 December 2024, Last revised 11 January 2026"
abstract: >
...
limma reads the peptide or peptide-precursor data from popular software tools such as DIA-NN (Demichev et al 2020), Spectronaut (https://staging.biognosys.com), FragPipe (Yu et al 2023), or MaxQuant (Tyanova et al 2016).
...
limpa includes the functions `readDIANN()`, `readSpectronaut()`, `readFragPipe()` and `readMaxQuant()`, which read peptide or peptide-precursor level data output by the popular mass spectrometry quantification tools DIA-NN, Spectronaut, FragPipe, and MaxQuant respectively.
...
`dpcQuant()` is fast for small to moderate datasets, but can be relatively slow, taking several hours, for very large datasets with thousands of samples.
For very large datasets, fast imputation can be done by `imputeByExpTilt()`, although this method does not provide standard errors to quantify precision.
...
* [Using limpa and limma to detect differential peptide usage](https://smythlab.github.io/limpa/DiffPeptideUsage.html)
